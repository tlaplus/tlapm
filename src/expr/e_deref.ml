...
let has_negix =
    ...
    fun e ->
        try
            vtor#expr ((), Deque.empty) e;
            false
        with Negix -> true


exception Label_found of
    hyp Deque.dq * expr * (Util.hint * int) list
...
let find_label =
    let scanner = object (self: 'self)
        ...
        method expr (l, cx as scx) e =
            match e.core with
            | Parens (e, {core = Xlabel (ll, llargs)}) ->
                if l = ll
                    then raise (Label_found (cx, e, llargs))
                    else ()
            | _ ->
                super#expr scx e

        method sequent (l, cx as scx) sq =
            ...
            | Some _ ->
                (* no visible labels underneath
                declarations or definitions *)
                scx
        end in
    fun e l -> scanner#expr (l, Deque.empty) e;
    raise No_such_label
...
let recx cx ds fmt =
    let hs = List.map
        begin fun df ->
            Defn (df, User, Visible, Export) @@ df
        end
        ds in
...
let rec deref
    ...
    | sel :: sels ->
        let e = if islab
            then e
            else paren_clean e in
        let (nvss, nds, e, sels, islab) =
            match sel with
            | Sel_down -> begin
                match e.core, sels with
                | Let _, (Sel_lab (opname, opargs) :: sels)
                        when islab ->
                    let (nvss, nds, e) = deref_letop
                        cx (List.length ds, e) opname opargs in
            ...
            | Sel_num n ->
                let (nds, e) =
                    ...
                    with Bad_index ->
                        Errors.warning := true;
                        Errors.set orig_e (Printf.sprintf
                            "Cannot resolve !%d" n);
                        Util.eprintf ~at:orig_e
                            "Cannot resolve !%d" n;
            ...
            | Sel_lab (l, args) ->  begin
                ...
                | Let _
                        when not islab ->
                    let (nvss, nds, e) = deref_letop
                        cx (List.length ds, e) l args in
                    (nvss, nds, e, sels, true)
                | _ ->
                    let (nvss, nds, e) = deref_label
                        cx (List.length ds, e) l args in
                    (nvss, nds, e, sels, true)
                end
            | Sel_at ->
                let (nvss, e) = deref_at
                    cx (List.length ds, e) in
                (nvss, [], e, sels, false)
            in
        let sels = List.map
            (app_sel (shift (List.length nds))) sels in
        deref ~islab:islab
            cx (vss @ nvss, ds @ nds, e) sels
    ...
    | Let (lds, _) ->
        let rec find_def prefix = function
            ...
            | ld :: lds -> begin
                match ld.core with
                | Operator (nm, op)
                        when nm.core = l -> begin
                    let op = match op.core with
                        | Lambda (vss, e) ->
                            let vsslen = List.length vss in
                            let largs = List.mapi
                                (fun n (v, _) -> (v, vsslen - n))
                                vss in
                            let e = Parens (
                                e, Xlabel (l, largs) @@ nm) @@ e in
                            Lambda (vss, e) @@ op
                        | _ ->
                            Parens (
                                op, Xlabel (l, []) @@ nm) @@ op
...
and deref_num cx (ds, e) n =
    assert (n > 0);
    let check_lteq k = if n > k
        then raise Bad_index in
    ...
    if is_badexp se then begin
        Errors.warning := true;
        Errors.set e  (Printf.sprintf
            "Cannot resolve subexpression reference !%d" n);
        Util.eprintf ~at:e
            "Cannot resolve subexpression reference !%d" n;
        failwith "Expr.Deref.deref_num"
    end;
...
and deref_inst cx e args =
    ...
    | Lambda (vss, le) ->
        ...
        deref_inst cx e args
    | ( SetSt (_, _, bod)
      | Choose (_, _, bod) ) -> begin
        match args with
        | [arg] -> app_expr
            (scons (assign arg atomic ()) (shift 0)) bod
            (* was shift 1 -> resolves tlapm bug
            18 Oct 2010 #1 *)
        | _ ->
            Errors.warning := true;
            Errors.set e (Printf.sprintf
    ...
    | _ ->
        Errors.warning := true;
        Errors.set e
            "Cannot instantiate non-binding expression";
        Util.eprintf ~at:e
            "Cannot instantiate non-binding expression";
        failwith "Expr.Deref.deref_inst"
...
and deref_label cx (dslen, e) l largs =
    ...
    with
        ...
        | Label_found (lcx, e, lpi) ->
            if List.length lpi <> List.length largs &&
                    largs <> [] then begin
                Errors.warning := true;
                Errors.set e (Printf.sprintf
            ...
            let (vss, largs) =
                match lpi, largs with
                | _, [] ->
                    let vss = List.mapi
                        (fun n (v, _) -> (v, Shape_expr))
                        lpi in
                    let largs = List.rev
                        (List.mapi
                            (fun n _ -> Ix (n + 1) @@ e)
            ...
            let rec reify_cx cx e =
                ...
                | Some (cx, {core=Fresh (v, _, _, _)}) ->
                    let bounds = [
                        E_t.From_hint.make_const_decl v] in
                    let forall = E_t.From_hint.make_forall
                        bounds e in
            ...
            let perm xs es =
                ...
                let xs = List.map
                    (fun k -> xsl - k + 1) xs in
                List.map (fun k -> List.nth es (k - 1)) xs in
            let largs = perm lpi largs in
            let rec apply_largs e largs =
                match largs with
                | [] -> ([], e)
                | larg :: largs -> begin
                    match e.core with
