    ...
    method expr scx e =
            ...
            let rec check
                ...
                | Some (dep,
                    ...
                    | _ -> begin
                        let op = app_expr
                            (shift (dep + 1)) op in
                        let op = Util.set_locus
                            op (Util.get_locus e) in
                        let se = Deref.resolve_bang
                ...
                | Some _ ->
                    ...
                    Util.eprintf ~at:e "%s"
                        ("invalid subexpression reference in:  " ^
                         expr_str);
                    Errors.set e (Printf.sprintf
                        "invalid subexpression reference");
        ...
        | Apply ({core = Opaque name}, args) ->
            ...
            if List.mem name (fst scx) then begin
                ...
                match pair with  (* Replace operator application
                    with the de Bruijn index of the operator.
