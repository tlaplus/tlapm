...
and plhdr = string
    (** Placeholder (for refinement predicates) *)
...
(** Makes a trivial refinement type
with base type [t] *)
let mk_ref t = Ref (
    fresh_tyterm (),
    t,
...
let add_x_ref_last x t = function
    | Ex (cx, e) ->
...
(** Apply explicit substitutions [ss]
to refinement predicate [cx, e] *)
...
let rec subst_reduce ss t: t =
    ...
    *)
    ...
    | ss, Ref (x, t, Ex (cx, e)) ->
        let cx, e = add_exp_substs
            cx e ss in  (** FIX it should need env *)
        Ref (x, t, Ex (cx, e))
...
let rec subst a x = function
    ...
    | Rec rs ->
        Rec (map
            (fun (h, t) -> (h, subst a x t))
            rs)
    | Rec_dot (t, h) ->
        Rec_dot (subst a x t, h)
    | TyPlus ts ->
        TyPlus (map (subst a x) ts)
    | TyTimes ts ->
        TyTimes (map (subst a x) ts)
    | Tbase t ->
        Tbase (subst a x t)
    | Tdom t ->
        Tdom (subst a x t)
...
let rec vsubst a b = function
    ...
    | Rec_dot (t, h) ->
        Rec_dot (
            ...
            h)
    | TyPlus ts ->
        TyPlus (map (vsubst a b) ts)
    | TyTimes ts ->
        TyTimes (map (vsubst a b) ts)
    | Tbase t ->
        Tbase (vsubst a b t)
    | Tdom t ->
        Tdom (vsubst a b t)
    ...
    | t ->
        t
and ss_vsubst a b = map
    (fun (v, cx, e, t) ->
        (if a = v then b else v),
        cx,
        e,
        vsubst a b t)
    ...
    try
        for_all2
            (fun (v, cx, e, t)
                    (v', cx', e', t') ->
                (*
                (if length cx <> length cx'
                    then Printf.eprintf
                        "** Warning! Contexts
                        don't match for delayed
                        substitutions.\n");
                *)
                (* cx_eq cx cx' && *)
                v = v' &&
...
let pp_cx ppf cx =
    let cx = mapi
        (fun i k -> Smt.lookup_id
            cx (i + 1))
        cx in
    Util.eprintf
        "@[<hov>%a@]"
        (pp_print_delimited pp_print_string)
        cx
*)
...
let rec simplify t =
    ...
    *)
    | Rec rs ->
        Rec (map
            (fun (h, t) -> (h, simplify t))
            rs)
    | Rec_dot (t, h) ->
        let t = simplify t in
        begin
        match t with
        (** Rec_dot (Rec [h_i -> t_i], h) = t_i
        when h = h_i *)
        | Rec rs ->
            begin
            try
    ...
    | TyPlus ts ->
        ...
        | [t1; t2]
            ...
            t1
        | [Ref (x, t1, Ex (cx, _));
           Ref (y, t2, Ex (cx', {core=Internal B.TRUE}))]
        | [Ref (x, t1, Ex (cx, {core=Internal B.TRUE}));
           Ref (y, t2, Ex (cx', _))]
                when eq t1 t2 ->
            Ref (
                x,
                t1,
                Ex (cx, Internal B.TRUE %% []))
        | [Ref (x, t1, Ex (cx, e1));
           Ref (_, t2, Ex (_, e2))]
                when eq t1 t2 ->
            Ref (
        ...
        | _ ->
            ...
            try
                ...
                let es = map
                    ...
                    ts
                    |> List.rev in
                let x, b, cx = match List.hd ts with
                    (x, t, cx, ex) -> x, t, cx in
                Ref (
                    x,
                    b,
    ...
    *)
    | Rec rs ->
        Rec (map
            ...
            rs)
    | Rec_dot (t, h) ->
        Rec_dot (base_to_ref t, h)
    | TyPlus ts ->
        TyPlus (map base_to_ref ts)
    | TyTimes ts ->
        TyTimes (map base_to_ref ts)
    | Tbase t ->
        Tbase (base_to_ref t)
    | Tdom t ->
        Tdom (base_to_ref t)
    | Tcod t ->
        Tcod (base_to_ref t)
    | t -> t
and base_to_ref_ss ss = map
    (fun (v, cx, e, t) ->
        v, cx, e, base_to_ref t)
    ss
    ...
    *)
    | Rec rs ->
        Rec (map
            ...
            rs)
    | Rec_dot (t, h) ->
        Rec_dot (ref_to_base t, h)
    | TyPlus ts ->
        TyPlus (map ref_to_base ts)
    | TyTimes ts ->
        TyTimes (map ref_to_base ts)
    | Tbase t ->
        Tbase (ref_to_base t)
    | Tdom t ->
        Tdom (ref_to_base t)
    | Tcod t ->
        Tcod (ref_to_base t)
    | t -> t
and ref_to_base_ss ss = map
    (fun (v, cx, e, t) ->
        v, cx, e, ref_to_base t)
    ss
...
From assignment [x : t],
it constructs the typing proposition [x \in t'].
[[x : t]] == x \in t'
It returns:
    [t']
    [cx']
...
let rec to_predtyp cx (x: expr) t =
    ...
    *)
    ...
    | TyAtom t' ->
        TyAtom t',
        cx,
        Apply (Opaque t' |> mk, [x]) |> mk
    (** [[x : a]] == a(x) *)
    | TyVar ([], a) ->
        (** this case represent atomic types *)
        TyVar ([], a),
        cx,
        Apply (Opaque a |> mk, [x]) |> mk
    | TyVar (ss, a) ->
        TyVar (ss, a),
        cx,
        Apply (Opaque a |> mk, [x]) |> mk  (** FIX *)
    | Top ->
        t,
        cx,
        Internal B.TRUE |> mk
    ...
    | Int ->
        t,
        cx,
        mem x (Internal B.Int |> mk)
    (** [[x : Int]] == x \in BOOLEAN *)
    | Bool ->
        t,
        cx,
        mem x (Internal B.BOOLEAN |> mk)
    | Str ->
        t,
        cx,
        mem x (Internal B.STRING |> mk)
    ...
    *)
    | Func (y, t1, t2) ->
        ...
        (*
        let b2, cx2, p2 = to_predtyp
            cx ix2 t2 in
        *)
        let b3, cx3, p3 = to_predtyp
            cx1
            (FcnApp (x, [ix1]) %% [])
            t2 in
        Func ("", b1, b3), cx, lAnd [
    ...
    | Ref (_, t, Ex (cx, e)) ->
        ...
        t,
        cx,
        lAnd [p; app_expr (scons x (shift 0)) e]
    | Ref (_, _, Ph _) ->
...
let rec get_type scx e: t option =
    ...
    *)
    (*
    let _is_int e = get_type
        scx e = Int in
    *)
    let oget scx e = Option.default
        Top (get_type scx e) in
    match e.core with
    | Ix n ->
        lookup_type scx n
    | Opaque id ->
    ...
    | FcnApp (f, es) ->
        ...
        let ts' = try
                Some (map Option.get ts)
            with _ ->
                None in
        begin
        match t, ts' with
        | Some t, Some ts ->
            Some (reduce_func t ts)
        | _ -> None
        end
        *)
        None
...
let rec is_int scx e =
    ...
    | Ix _ -> _is_int scx e
        ...
        assert (
            n > 0 &&
            n <= Ctx.length cx);
        let id = Ctx.string_of_ident
            (fst (Ctx.index cx n)) in
        id
        *)
    | Apply ({core=Internal op}, [e1; e2]) ->
        ...
        | B.Exp ->
            ...
            is_int scx e2
        (** This works with the
        encoding hack as IFs,
        because the condition
        [e1 \in Int /\ e2 \in Int /\ 0 < e2]
...
let rec is_hard_bool e =
    ...
    *)
    ...
    | Apply ({core=Opaque "boolify"}, _) ->
        true
    | Apply ({core=Opaque "tla__isAFcn"}, _) ->
        true
    | If (_, t, f) ->
        is_hard_bool t &&
        is_hard_bool f
