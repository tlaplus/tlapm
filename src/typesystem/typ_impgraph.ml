...
let simp_list e =
    match e.core with
    | List (And, es) ->
        List (And, Smt.remove_repeated_ex es) %% []
    | List (Or, es) ->
        List (Or, Smt.remove_repeated_ex es) %% []
    | _ -> e
...
let in_fm = ref SMap.empty
    (** node [i] |->
        list of formulas
        pointing to formula [i] *)
let in_ph = ref SMap.empty
    (** node [i] |->
        list of formulas
        pointing to placeholder [i] *)
let out_fm = ref SMap.empty
    (** node [i] |->
        list of formulas to
        which formula [i] points *)
let out_ph = ref SMap.empty
    (** node [i] |->
        list of formulas to
...
let madd m j ps =
    if ps = []
        then ()
        else m := SMap.add j (ps @ (!m @@ j)) !m

let mdel m j x =
    let xs = Smt.subtract (!m @@ j) x in
    m := if xs = []
        then SMap.remove j !m
        else SMap.add j xs !m
...
let mk_imp_graph vcs =
    ...
    | B.Implies,
      Ph (_, p),
      Ex (cx, e) ->
        madd out_fm p [Typ_e.to_cx env, e]
    | B.Implies,
      Ex (cx, e),
      Ph (_, p) ->
        madd in_fm p [Typ_e.to_cx env, e]
    | _ -> ()
    in
    iter add1 vcs
...
let rule1 k =
    ...
    let cx, ex =
        let cx =
            try
                let cx, _ = hd (!out_fm @@ k) in
                cx
            with _ ->
                [] in
        let es = List.map
            (fun (cx, e) -> e)
            (!out_fm @@ k) in  (** FIX cx not used *)
        (* let es = rev es in *)
        let es =
            try
                let cx, ex = SMap.find
                    k !plhdrs in
                ex :: es  (** FIX cx not used *)
            with Not_found ->
                es in
        let ex = List (And, es)
            |> mk_list in
...
let rule2 k =
    ...
    let cx, ex =
        let cx =
            try
                let cx, _ = hd (!in_fm @@ k) in
                cx
            with _ ->
                [] in
        let es = List.map
            (fun (cx, e) -> e)
            (!in_fm @@ k) in  (** FIX cx not used *)
        (* let es = rev es in *)
        (** new formula *)
        let es =
            try
                let cx, ex = SMap.find
                    k !plhdrs in ex :: es  (** FIX cx not used *)
            with Not_found ->
                es in
        let ex = List (Or, es)
            |> mk_list in
...
let rec fix ns c rule x =
    let eq p q = SMap.equal
        (fun (cx, e) (cx', e') ->
            Expr.Eq.expr e e') p q in
    if c = 0
        then
            (failwith
...
let rule_true i =
    if !in_ph @@ i <> [] then begin
        ...
        iter
            (fun j -> plhdrs := SMap.add j e !plhdrs)
            (i :: ps)
    end else ()
...
let solve init_phs vcs ns =
    ...
    *)
    ...
    iter rule_true ns;  (** Undefined
        placeholders are assigned TRUE *)
