...
let create_vars id ts t  (* : Tm.term  *) =
    ...
    match ts, t with
    | [], _ ->
        let fsym = Tm.create_fsymbol
            (Ident.id_fresh id) [] t in
        (fsym, t, Tm.fs_app fsym [] t)
    (*
    | _, t'
            when Ty.ty_equal t' Ty.ty_bool ->
        Tm.ps_app (
            Tm.create_psymbol
            (Ident.id_fresh id)
            ts)
...
let rec to_why cx e : Tm.term =
    ...
    *)
    let tm_ix id = begin
        try
            let fsym, t, tm = SMap.find
                id !var_terms in
            tm
        with Not_found ->
            failwith
                "not a declared variable id"
        end in
    match e.core with
    | Ix n ->
        tm_ix (lookup_id cx n)
    | Opaque id ->
        tm_ix id
    (*
    | Apply ({core=Ix n}, es) ->
    ...
    *)
    ...
    | Apply ({core=Internal op}, es) ->
        ...
        | B.Neg, [ex] ->
            Tm.t_not ex
        | B.Mem, [e1; e2] ->
            Tm.ps_app mem_symbol es
        (* | B.DOMAIN, [f] -> *)

        | B.Plus, [e1; e2] ->
            Tm.t_app_infer plus_symbol es
        | B.Minus, [e1; e2] ->
            Tm.t_app_infer minus_symbol es
        | B.Times, [e1; e2] ->
            Tm.t_app_infer mult_symbol es
        (*
        | B.Exp, [e1; e2] ->
            Tm.t_app_infer exp_symbol es
        *)
        (* | B.Ratio, [e1; e2] *)
        | B.Lt, [e1; e2] ->
            Tm.ps_app lt_symbol es
        | B.Lteq, [e1; e2] ->
            Tm.ps_app lteq_symbol es
        | B.Gt, [e1; e2] ->
            Tm.ps_app gt_symbol es
        | B.Gteq, [e1; e2] ->
            Tm.ps_app gteq_symbol es
        (* | B.Quotient, [e1; e2] -> *)
        (* | B.Remainder, [e1; e2] -> *)
        (* | B.Quotient, [e1; e2] -> *)
        (* | B.Remainder, [e1; e2] -> *)
        ...
        (*
        | B.Uminus, [ex] ->
            Tm.t_app_infer uminus_symbol es
        *)
        (*
        | B.Seq, [ex] ->
            proc_op "Seq" es
        | B.Len, [ex] ->
            output (proc_op "Len" es)
        | B.SubSeq, [ex; m; n] ->
            proc_op "SubSeq" es
        | B.Head, [ex] ->
            proc_op "Head" es
    ...
    *)
    ...
    (* | Tuple es -> *)
    | Num (n, "") ->
        Tm.t_const (Number.ConstInt
            (Number.int_const_dec n))
    (* | Internal B.Nat -> *)
    (* | Internal B.Int -> *)
    (* | Internal B.Infinity -> *)
    | If (c, t, f) ->
...
(*
let term_lsymbols env = SMap.mapi
    (fun x _ -> x) env
*)
...
let rec normalize cx e =
    ...
    | Apply (
        ...
        (* map_exp normalize cx *)
        (Apply (Internal B.Eq |> noprops, [x; y])
            |> noprops)
...
let solve ((env:Typ_e.t), e) =
    ...
    let vs = map
        (fun (x, (fsym, _, _)) -> fsym)
        (SMap.bindings !var_terms) in
    let e = normalize [] e in
    let fm = to_why (Typ_e.to_cx env) e in
    (*
    Util.eprintf
        "@[   Why3 formula:@ %a@]@."
        Pretty.print_term fm;
    *)
