(* Constraint generation (CG) for type system with refinement types.
...
let mk_eq_bool t = if t = Bool
    then CAtom CTrue
    else mk_eq (E.empty, t, Bool)
...
let rec cg_expr
    ...
    Util.eprintf
        "<< %a|- %a : %a >>"
        E.pp env (E.pp_print_expr (E.to_scx env, Ctx.dot))
        e E.ppt (env, t);
    *)
    ...
    let app op x y = Apply (
        Internal op %% [],
        [x; y]) %% [] in
    let apps op xs = Apply (
        Internal op %% [],
        xs) %% [] in
    let ix1 = Ix 1 %% [] in
    let sh1 e = app_expr (shift 1) e in
    let mk_ex op es = Apply (
        Internal op %% [],
        es) @@ e in
    ...
    | FcnApp (f, [ex]) ->
        ...
        *)
        ...
        Util.eprintf
            "<< ___ |- %a : %a >>"
            (E.pp_print_expr (E.to_scx env, Ctx.dot))
            e E.ppt (env, t);
        *)
        (*
        Util.eprintf
            "<< ___ |- %a : %a >>"
            (E.pp_print_expr (snd scx, Ctx.dot))
            (sh_diff ex) E.ppt (env, t);
        *)
        let af = fresh_tyvar (E.to_cx env, f) in
        let ae = fresh_tyvar (E.to_cx env, ex) in
        let f, cf = cg_expr
            mode env (mk_var af) scx f in
        let ex, cex = cg_expr
            mode env (mk_var ae) scx ex in
    ...
    | Fcn (bs, ex) ->
        let scx', bs, cbs, a1s, a2s, vs, env', local_env = cg_bounds
            env scx bs in
        let a2s = List.map Option.get a2s in
        (*
        let css = map2
            (fun a1 a2 -> mk_sub (env, mk_var a2, mk_var a1))
            a1s a2s in  (** \a_x <: \a_S *)
        *)
        let ae = fresh_tyvar (E.to_cx env', ex) in
        let ex, cex = cg_expr
        ...
        *)
        ...
        let tr = Ref (
            ...
            mk_var ae,
            Ex (
                add_x_ctx z (mk_var ae) (E.to_cx env),
                app B.Eq ix1 (sh1 ix1))) in
        *)
        let tr = mk_var ae in
        let tt = fold_right2
            (fun v a r -> Func (v, mk_var a, r))
    ...
    | Apply ({core=Internal op}, es) ->
        ...
        | B.Cup, [e1; e2] ->
            ...
            let e1, ce1 = cg_expr
                mode env (Set (mk_var a1)) scx e1 in
            let e2, ce2 = cg_expr
                mode env (Set (mk_var a2)) scx e2 in
            mk_ex op [e1; e2],
            CExists (
                [a1; a2],
                CConj [
        ...
        | B.SUBSET, [s] ->
            ...
            let s, cs = cg_expr
                mode env (Set (mk_var a)) scx s in
            mk_ex op [s],
            CExists (
                [a],
                CConj [
        ...
        *)
        | B.Range, [x; y] ->
            ...
            *)
            ...
            Util.eprintf
                "<< ___ |- %a : %a >>"
                (E.pp_print_expr (snd scx, Ctx.dot))
                (sh_diff e) E.ppt (env, t);
            *)
            let ax = fresh_tyvar (E.to_cx env, x) in
            let ay = fresh_tyvar (E.to_cx env, y) in
            let x, cx = cg_expr
                mode env (mk_var ax) scx x in
            let y, cy = cg_expr
                mode env (mk_var ay) scx y in
            let z = fresh_tyterm () in
            mk_ex op [x; y],
            CExists (
                [ax; ay],
        ...
        | (B.Conj | B.Disj | B.Implies | B.Equiv), [e1; e2] ->
            let e1, ce1 = cg_expr
                mode env Bool scx e1 in
            let e2, ce2 = cg_expr
                mode env Bool scx e2 in
            mk_ex op [e1; e2],
            CConj [
                ce1;
                ce2;
        ...
        *)
        | B.Eq, [e1; e2] when mode = TypHyp ->
            ...
            let e1, ce1 = cg_expr
                OnlySafe env (mk_var a) scx e1 in
            let e2, ce2 = cg_expr
                OnlySafe env (mk_var a) scx e2 in
            mk_ex op [e1; e2],
        ...
        *)
        | (B.Eq | B.Neq), [e1; e2] ->
            ...
            let a2 = fresh_tyvar (E.to_cx env, e2) in
            let e1, ce1 = cg_expr
                mode env (mk_var a1) scx e1 in
            let e2, ce2 = cg_expr
                mode env (mk_var a2) scx e2 in
            mk_ex op [e1; e2],
            CExists ([a1;a2], CConj [
                ce1;
                mk_issub (
        ...
        | B.Mem, [e1; e2]
            ...
            let a2 = fresh_tyvar (E.to_cx env, e2) in
            let e1, ce1 = cg_expr
                OnlySafe env (mk_var a1) scx e1 in
            let e2, ce2 = cg_expr
                OnlySafe env (Set (mk_var a2)) scx e2 in
            mk_ex op [e1; e2],
            CExists (
                [a1; a2],
                CConj [
        ...
        | (B.Plus | B.Minus | B.Times | B.Exp |
           B.Ratio | B.Quotient | B.Remainder), [x; y] ->
            ...
            let x, cx = cg_expr
                mode env (mk_var a1) scx x in
            let y, cy = cg_expr
                mode env (mk_var a2) scx y in
            let z = fresh_tyterm () in
            mk_ex op [x; y],
            CExists (
                [a1; a2],
    ...
    *)
    ...
    | Apply (
        ...
        let a1s, t1s, es, cs = gen_tyvars
            mode env scx es in
        let a2s = List.map
            (fun e -> fresh_tyvar (E.to_cx env, e))
            es in
        let t2s = List.map
            (fun a -> mk_var a)
            a2s in
    ...
    *)
    | SetSt (v, d, ex) ->
        ...
        let d, cd = cg_expr
            mode env (Set (mk_var a)) scx d in
        let scx' = adj scx
            (Fresh (
                v,
                Shape_expr,
                Constant,
                Bounded (d, Visible)) @@ v) in
        let local_env = E.adj
            empty (v.core, mk_var a) in
    ...
    | SetOf (ex, bs) ->
        let scx', bs, cbs,
            a1s, a2s, vs,
            env', local_env = cg_bounds
                env scx bs in
        let a2s = List.map Option.get a2s in
        let ae = fresh_tyvar (E.to_cx env', ex) in
        let ex, cex = cg_expr
            mode env' (Set (mk_var ae)) scx' ex in
        SetOf (ex, bs) @@ e,
        CExists (a1s @ a2s @ [ae],
            CConj (
                cbs @ [
    ...
    | Arrow (x, y) ->
        ...
        let a2 = fresh_tyvar (E.to_cx env, y) in
        let x, cx = cg_expr
            mode env (Set (mk_var a1)) scx x in
        let y, cy = cg_expr
            mode env (Set (mk_var a2)) scx y in
        Arrow (x, y) @@ e,
        CExists (
            [a1; a2],
            CConj [
    ...
    | If (c, v, u) ->
        ...
        let c, cc = cg_expr
            mode env (mk_var a1) scx c in
        let v, cv = cg_expr
            mode env (mk_var a2) scx v in
        let u, cu = cg_expr
            mode env (mk_var a3) scx u in
        If (c, v, u) @@ e,
        CExists (
            [a1; a2; a3],
            CConj [
    ...
    | Except (f, [([Except_apply a], b)]) ->
        ...
        Util.eprintf
            "<< %a|- %a : %a >>"
            E.pp env (E.pp_print_expr (snd scx, Ctx.dot))
            e E.pp (env, t);
        *)
        ...
        let f, cf = cg_expr
            mode env (mk_var a1) scx f in
        let a, ca = cg_expr
            mode env (mk_var a2) scx a in
        let b, cb = cg_expr
            mode env (mk_var a3) scx b in
        (** [ix1] is [z], [ix2] is [x] *)
        (*
        let ex =
            let ix2 = Ix 2 %% [] in
            ...
            (sh1 (sh1 b)),
            FcnApp (sh1 (sh1 f), [ix2]) %% [])) %% []) in
        *)
        (** [ix1] is [z], [ix_last] is [x] *)

        let ix_last = Ix (Deque.size (snd scx) + 2) %% [] in
        let e1 = apps B.Neg
            [app B.Eq ix_last (sh_diff (sh1 a))] in
        let e2 = app B.Conj (
            app B.Eq ix_last (sh_diff (sh1 a)))
            (app B.Eq ix1 (sh_diff (sh1 b))) in
    ...
    | Tuple [] ->
        ...
        mk_eq (
            ...
            Func ("", tt, tt))
    | Tuple es ->
        let aas, ats, es, cs = gen_tyvars
            mode env scx es in
        let i = fresh_tyterm () in
        let n = List.length es in
        let x1 = fresh_tyterm () in
        let cx' = add_x_ctx
            x1 Int (E.to_cx env) in
        let dom = Ref (
            x1,
            Int,
            Ex (
    ...
    | Internal B.BOOLEAN ->
        e,
        mk_eq (E.empty, t, Set Bool)

    (** Constraint Generation for
    Boolean expressions [env |- (cx, e) : Bool] *)
    | Internal B.TRUE
    | Internal B.FALSE ->
        e,
        mk_eq_bool t
                ...
                [{core=Apply (
                 ...
                 s2])})
                    when mode = TypHyp
                        (* TOFIX: and [x \notin FV(T)] *) ->
        (*
        let _, bs, cbs, a1s, a2s, vs, _, local_env = cg_bounds
            env scx bs in
        *)
        (*
        let a2s = fold_left
            (fun r -> function Some a -> a :: r | None -> r)
            [] a2s in
        *)
        let a1 = fresh_tyvar (E.to_cx env, s1) in
        let a2 = fresh_tyvar (E.to_cx env, s2) in
        let s1, cs1 = cg_expr
            mode env (Set (mk_var a1)) scx s1 in
        let s2, cs2 = cg_expr
            mode env (Set (mk_var a2)) scx s2 in
        let op, cop = cg_expr
            mode env
            (Func ("", mk_var a1, mk_var a2))
            scx op in
    ...
    *)
    | Quant (
        ...
        let s, cs = cg_expr
            mode env (Set (mk_var a2)) scx s in
        let ip = Smtcommons.lookup_id
            (E.to_cx env) (n - 1) in
        let iq = Smtcommons.lookup_id
            (E.to_cx env) (m - 1) in
    ...
    | Quant (
        ...
        let a2 = fresh_tyvar (E.to_cx env, s) in
        let local_env = E.adj
            empty (h.core, mk_var a1) in
        let env' = env $! local_env in
        let px, cp = cg_expr
            mode env' Bool scx px in
        let s, cs = cg_expr
            mode env (Set (mk_var a2)) scx s in
        let opx, cop = cg_expr
            mode env' (mk_var a2) scx opx in
        let bs = [h, k, No_domain] in
        let ex = Quant (
            Forall,
            bs,
            app B.Implies px (app B.Mem opx s)) @@ e in
        ex,
        CExists (
            [a1; a2],
            CConj [
    ...
    *)
        ...
        let e, ces = match q, ex.core with
            ...
            | Forall, Apply (
                    ...
                    [{core=Apply (
                        ...
                        | List (And, hs)};
                    c]) ->
                let ths, rest = List.partition
                    (Smtcommons.is_typhyp (E.to_scx env))
                    hs in
                let ths, cths = split
                    (List.map
                        (cg_expr TypHyp env' Bool scx')
                        ths) in
                let rest, crest = split
                    (List.map
                        (cg_expr OnlySafe env' Bool scx')
                        rest) in
                let hs = ths @ rest in
            ...
            | Exists, (
                ...
                let hs = ths @ rest in
                let ex = lAnd hs
                    |> Smtcommons.flatten_conj in
                Quant (q, bs, ex) @@ e,
                (cths @ crest)
            | _ ->
                let ex, cex = cg_expr
                    OnlySafe env' Bool scx' ex in
                Quant (q, bs, ex) @@ e,
                [cex] in
    ...
    | List (b, es) ->
        let es, ces = split
            (List.map (cg_expr mode env Bool scx) es) in
        let e = List (b, es) @@ e in
        e,
        CConj (mk_eq_bool t :: ces)
    | Sequent sq ->
        (*
        let e, c = cg_expr
            mode env Bool scx (Smtcommons.unroll_seq sq) in
        e,
        CConj [
            mk_eq (
                E.empty,
...
(** Given a list of expressions [es],
in a context [scx], in a typing
...
and gen_tyvars
    ...
    let es, cs = split
        (map2
            (fun e a -> cg_expr
                mode env
                (if s then Set a else a)
                scx e)
            es ats) in
    aas, ats, es, cs

(** Given a list of bounds [bs],
in a context [scx], in a typing
environment [env], generate:
[scx] = updated [scx]
[bs] = updated [bs]
[cbs] = list of constraints in [bs]
[a1s] = list of new type variable ids
(alpha strings) for variables
[a2s] = list of type variable ids
    for domains
...
and cg_bounds env scx bs =
    ...
    let bs, cbs, a1s, a2s, vs =
        List.map
            begin fun (v, k, d) ->
                let a1 = fresh_tyvar
                    ~id:("_" ^ v.core)
                    (E.to_cx env, Opaque v.core @@ v) in
                match d with
                | Domain s ->
                    let a2 = fresh_tyvar
                        (E.to_cx env, s) in
    ...
    let env = fold_left2
        ...
        env vs a1s in
    *)
    let local_env = E.adjs
        empty
        (map2 (fun v a -> v, mk_var a) vs a1s) in
    let env' = env $! local_env in
    (adjs scx hs,
     bs,
     cbs,
...
(* let ph cx ff h = ignore (E.pp_print_hyp cx ff h) in *)
    ...
    | Fact (e, Visible, tm) ->
        ...
        Util.eprintf
            ...
            (Dq.size env)
            (Dq.size (snd scx))
            E.pp env (ph (E.to_scx env, Ctx.dot)) h;
        *)
        let mode = if Smtcommons.is_typhyp (E.to_scx env) e
            then TypHyp
            else OnlySafe in
        let e, ce = cg_expr
            mode env Bool scx e in
        ...
        let dom, c = match dom with
            | Bounded (r, Visible) ->
                let e = Apply (
                    Internal B.Mem %% [],
                    [Opaque nm.core @@ nm;
                     app_expr (shift 1) r]) %% [] in
                let e, c = cg_expr
                    TypHyp env' Bool scx e in
...
and cg_sequent env scx sq =
    ...
    Smtcommons.ifprint 3
        "<< [%d] %a@, |- %a : Bool >>"
        (Dq.size env) E.pp env Fu.pp_print_minimal
        (Fu.Big (fun ff -> ignore (
            E.pp_print_sequent
                (sq.context, Ctx.dot) ff sq)));
    *)
    (*
    Smtcommons.ifprint 3
        "<< [%d] %a@, |- %a : Bool >>"
        ...
        (Fu.Big (fun ff -> ignore (
            E.pp_print_sequent
                (snd scx, Ctx.dot) ff sq)));
    *)
    (*
    let scx, hs, env, vs, chs = cg_hyps
        env ((), Dq.empty) sq.context in
    *)
    let scx, hs, env, vs, chs = cg_hyps
        env scx sq.context in
    (*
    Smtcommons.ifprint 3
        "<< [%d] %a@, |- %a : Bool >>"
        (Dq.size env) E.pp env Fu.pp_print_minimal
    ...
    *)
    let c, cc = cg_expr
        OnlySafe env Bool scx sq.active in
...
let cg sq =
    ...
    *)
    ...
    (*
    let final_env, _ = Dq.fold_lft
        (fun e h -> e $$ (h, None))
        E.empty sq.context in
    *)
    (* let offset = Dq.size final_env in *)
    let _, sq, env, c = cg_sequent
    ...
    (*
    Smtcommons.ifprint 3
        "   Constraint length = %d" (C.c_length c);
    *)
    (*
    Smtcommons.ifprint 3
        "sq': %a"
