...
val eq : t -> t -> bool
val eqc : tc -> tc -> bool
val simplify : Typ_e.t * t -> (Typ_e.t * t)
(* val solve_residual_iseq : Typ_e.t -> t -> Typ_e.t * t *)
val subst : string -> Typ_t.t -> t -> t
val substc : string -> Typ_t.t -> tc -> tc
val vsubst : string -> string -> t -> t
val subs_ss : string -> Typ_t.t -> (string * Typ_t.t) list -> (string * Typ_t.t) list
val app_ss : (string -> Typ_t.t -> 'a -> 'a) -> 'a list -> (string * Typ_t.t) list -> 'a list
val apply_ss : (string * Typ_t.t) list -> (Typ_e.t * string list * t list) -> (Typ_e.t * string list * t list)
val fv : t -> string list
val fix : ('a -> 'a -> bool) -> ('a -> 'a) -> 'a -> 'a
val fix_env_c : (Typ_e.t * t -> Typ_e.t * t) -> (Typ_e.t * t) -> (Typ_e.t * t)
(* val pp_env_c : Typ_e.t -> t -> unit *)
(* val c_to_cset : (Typ_e.t * t) -> (Typ_e.t * string list * tc list) *)
(* val flatten_cconj : t -> t list *)
(* val simp0 : t -> t *)
val map_c : (t -> t) -> t -> t
val simp_c : t -> t
...
val rewrite_eqs : Typ_e.t * string list * t list -> Typ_e.t * string list * t list
val rw_c : (Typ_e.t * string list * t list -> Typ_e.t * string list * t list) -> Typ_e.t * t -> Typ_e.t * t
val mk_cs : t list -> t
val mk_ex : string list * t list -> t
val base_to_ref : t -> t
val iseq_to_eq : t -> t
val issub_to_sub : t -> t
...
val phs_log : (((string * Typ_t.t) * Typ_e.t * hyp list * expr) option) SMap.t ref
val new_ph : ((string * Typ_t.t) * Typ_e.t * hyp list * expr) option -> string
val pp_ph : string -> (hyp list * expr) -> unit
