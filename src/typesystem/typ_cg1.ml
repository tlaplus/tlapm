...
let mk_eq_cond (t1, t2) = if T.eq t1 t2
    then CAtom CTrue
    else mk_eq (t1, t2)
...
let rec cg_expr
    ...
    Util.eprintf
        ...
        e
        (if Smtcommons.apply_u2bool e then "^bool" else "")
        E.ppt (env, t);
    *)
    match e.core with
    ...
    | Apply ({core=Opaque "boolify"}, [{core=Apply (f, es)}]) ->
        ...
        CExists (
            ar :: a1s @ a2s,
            CConj (
                cs @ (List.map2 (fun a b -> mk_iseq (a, b)) t1s t2s)
    ...
    *)
    | Opaque "boolify" ->
        ...
        CExists (
            [a],
            mk_eq (t, Func ("", mk_var a, Bool)))
    | Opaque "bool2u" ->
        let a = fresh_tyvar (E.to_cx env, e) in
        e,
        CExists (
            [a],
            mk_eq (t, Func ("", Bool, mk_var a)))
    | Opaque "tla__isAFcn" ->
        let a = fresh_tyvar (E.to_cx env, e) in
        e,
        CExists (
    ...
    | Opaque "tla__fcnapp" ->
        ...
        CExists (
            [a1; a2],
            mk_eq (t, Func ("", mk_var a1, mk_var a2)))
    | Opaque (
              "tla__plus"
            | "tla__minus"
            | "tla__times"
        ...
        CExists (
            ...
            mk_eq (
                ...
                Func (
                    "",
                    mk_var a1,
                    Func ("", mk_var a2, mk_var a3))))
    | Ix n ->
        let id = Smtcommons.lookup_id (E.to_cx env) n in
        e,
        mk_eq (t, E.find id env)
    | Opaque s ->
        e,
        mk_eq (t, E.find s env)
    | FcnApp (f, [ex]) ->
    ...
    | FcnApp (f, [ex]) ->
        ...
        CExists (
            ...
            CConj [
                ...
                mk_iseq (
                    mk_var af,
                    Func ("", mk_var ae, mk_var ar));
                mk_eq (t, mk_var ar)])
    *)
    | Fcn (bs, ex) ->
        let scx', bs, cbs, aas, vs, env' = cg_bounds
            env scx bs in
        let ae = fresh_tyvar (E.to_cx env', ex) in
        let ex, cex = cg_expr
            ts env' (mk_var ae) scx' ex in
        let tt = fold_right
            (fun a r -> Func ("", mk_var a, r))
            aas
            (mk_var ae) in
        Fcn (bs, ex) @@ e,
        CExists (
    ...
    | Apply ({core=Internal op}, es) ->
        ...
        | B.UNION, [s] ->
            let s, cs = cg_expr
                ts env (Set t) scx s in
            mk_ex op [s],
            cs
        | B.DOMAIN, [f] ->
            let af = fresh_tyvar (E.to_cx env, f) in
            let f, cf = cg_expr
                ts env (mk_var af) scx f in
            mk_ex op [f],
            CExists (
                [af],
                CConj [
                    cf;
                    mk_eq (t, Set (Tdom (mk_var af)))])
        (*
        | B.DOMAIN, [f] ->
            let af = fresh_tyvar (E.to_cx env, f) in
            let a1 = fresh_tyvar (E.to_cx env, e) in
            let a2 = fresh_tyvar
                (E.to_cx env, (noprops (Opaque "dummy"))) in
            let f, cf = cg_expr
                ts env (mk_var af) scx f in
            mk_ex op [f],
            CExists (
        ...
        *)
        | B.Range, [x; y] ->
            ...
            let x, cx = cg_expr
                ts env (mk_var ax) scx x in
            let y, cy = cg_expr
                ts env (mk_var ay) scx y in
            mk_ex op [x; y],
            CExists (
                [ax; ay],
                CConj [
        ...
        | (B.Conj | B.Disj | B.Implies | B.Equiv), [e1; e2] ->
            let e1, ce1 = cg_expr
                ts env Bool scx e1 in
            let e2, ce2 = cg_expr
                ts env Bool scx e2 in
            mk_ex op [e1; e2],
            CConj [
                ce1;
                ce2;
        ...
        | B.Eq, [e1; e2]
            ...
            let e2, ce2 = cg_expr
                OnlySafe env (mk_var a2) scx e2 in
            mk_ex op [e1; e2],
            CExists (
                [a1; a2],
                CConj [
        ...
        | B.Subseteq, [e1 ; e2] ->
            ...
            let a2 = fresh_tyvar (E.to_cx env, e2) in
            let e1, ce1 = cg_expr
                ts env (Set (mk_var a1)) scx e1 in
            let e2, ce2 = cg_expr
                ts env (Set (mk_var a2)) scx e2 in
            mk_ex op [e1 ; e2],
            CExists (
                [a1; a2],
                CConj [
        ...
        | (       B.Plus
            ...
            let x, cx = cg_expr
                ts env (mk_var a1) scx x in
            let y, cy = cg_expr
                ts env (mk_var a2) scx y in
            mk_ex op [x; y],
            CExists (
                [a1; a2],
                CConj [
    ...
    | SetEnum es ->
        ...
        CExists (
            ...
            CConj (
                List.map
                    (cg_expr ts env (mk_var a) scx)
                    es @ [mk_eq (t, Set (mk_var a))]))
        *)
    | SetSt (v, d, ex) ->
        let a = fresh_tyvar (E.to_cx env, d) in
        let d, cd = cg_expr
            ts env (Set (mk_var a)) scx d in
        let scx' = adj scx
            (Fresh (
                v,
                Shape_expr,
    ...
    | SetOf (ex, bs) ->
        ...
        let ae = fresh_tyvar (E.to_cx env', ex) in
        let ex, cex = cg_expr
            ts env' (Set (mk_var ae)) scx' ex in
        SetOf (ex, bs) @@ e,
        CExists (
            aas @ [ae],
            CConj (
                cbs @ [cex; mk_eq (t, Set (mk_var ae))]))
    | String _ ->
        e,
        mk_eq_cond (t, Str)
    | Num _ ->
    ...
    | Internal B.STRING ->
        e,
        mk_eq_cond (t, Set Str)
    | Arrow (x, y) ->
        let a1 = fresh_tyvar (E.to_cx env, x) in
        let a2 = fresh_tyvar (E.to_cx env, y) in
        let x, cx = cg_expr
            ts env (Set (mk_var a1)) scx x in
        let y, cy = cg_expr
            ts env (Set (mk_var a2)) scx y in
        Arrow (x, y) @@ e,
        CExists (
            [a1; a2],
            CConj [
    ...
    | If (c, a, b) ->
        ...
        let c, cc = cg_expr
            ts env (mk_var a1) scx c in
        let a, ca = cg_expr
            ts env (mk_var a2) scx a in
        let b, cb = cg_expr
            ts env (mk_var a3) scx b in
        If (c, a, b) @@ e,
        CExists (
            [a1; a2; a3],
            CConj [
    ...
    | Dot (f, h) ->
        ...
        let f, cf = cg_expr
            ts env (mk_var a) scx f in
        Dot (f, h) @@ e,
        CExists (
            [a],
            CConj [
                cf;
                mk_eq (t, Rec_dot (mk_var a, h))])
    ...
    | Quant (q, bs, ex) ->
        ...
        CExists (
            aas,
            CConj (
                cbs @ [cex; mk_eq_cond (t, Bool)]))
    | List (b, es) ->
        let es, ces = split
            (List.map (cg_expr ts env Bool scx) es) in
        let e = List (b, es) @@ e in
        e,
        CConj (
            mk_eq_cond (t, Bool) :: ces)
    | Sequent sq ->
        let e, c = cg_expr
            ts env Bool scx (Smtcommons.unroll_seq sq) in
        e,
        CConj [
            mk_eq_cond (t, Bool);
            c]
...
(* let ph cx ff h = ignore (E.pp_print_hyp cx ff h) in *)
    ...
    | Fact (e, Visible, tm) ->
        ...
        *)
        let e, ce = cg_expr
            (if Smtcommons.is_typhyp (E.to_scx env) e then
                TypHyp
            else
                OnlySafe)
            env Bool scx e in
    ...
    | Flex nm ->
        ...
        Util.eprintf
            ...
            E.pp env (ph (E.to_scx env, Ctx.dot)) h;
        *)
        let a = fresh_tyvar ~id:("_" ^ nm.core)
            (E.to_cx env,
...
and cg_sequent env scx sq =
    (* Smtcommons.ifprint 3 "<< [%d] %a@, |- %a : Bool >>"
        (Dq.size env) E.pp env Fu.pp_print_minimal
        (Fu.Big (fun ff -> ignore (
            E.pp_print_sequent (snd scx, Ctx.dot) ff sq)));
    *)
...
let cg sq =
    ...
    (*
    Util.eprintf "CG: %a"
        Fu.pp_print_minimal (Fu.Big (fun ff -> ignore (
        E.pp_print_sequent (sq.context, Ctx.dot) ff sq)));
    *)
