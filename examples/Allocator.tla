...
LEMMA InitTypeInvariant == Init => TypeInvariant
...
LEMMA NextTypeInvariant == TypeInvariant /\ [Next]_vars => TypeInvariant'
...
THEOREM TypeOK == SimpleAllocator => []TypeInvariant
BY InitTypeInvariant, NextTypeInvariant, PTL DEF SimpleAllocator
...
<1>1. CASE Next
  BY <1>1, RequestMutexBis, AllocateMutex, ReturnMutex DEF Next
<1>2. CASE UNCHANGED vars
  BY <1>2 DEF Mutex, vars
<1>. QED  BY <1>1, <1>2

THEOREM MutualExclusion == SimpleAllocator => []Mutex
BY InitMutex, NextMutex, TypeOK, PTL DEF SimpleAllocator
