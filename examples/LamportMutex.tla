...
<1>2. TypeOK /\ [Next]_vars => TypeOK'
  ...
  <2>6. ASSUME NEW p \in Proc,
               ...
               ReceiveRelease(p,q)
        PROVE  TypeOK'
    BY <2>6, CVC4 DEF ReceiveRelease
    ...
    <3>4. ASSUME NEW p \in Proc
      ...
      <4>1. CASE p = n
        ...
        <5>. ASSUME NEW q \in Proc \ {n}
          ...
          BY <3>1, <3>2, <4>1, ContainsSend, NotContainsPrecedes, PrecedesSend, CVC4
             DEF Broadcast
      ...
      <4>1. CASE p = k
        ...
        <5>2. CASE q # n
          <6>. pq' = pq /\ qp' = qp /\ ack'[p] = ack[p]
            BY <3>1, <3>3, <4>1, <5>2
          <6>. CommInv(p)!2!3!(q)
            BY <3>3, <4>1, Zenon DEF BasicInv, CommInv
          <6>. QED  OBVIOUS
        <5>. QED  BY <5>1, <5>2
      <4>2. CASE p = n
      ...
      <4>1. CASE p = n
        ...
        <5>1. CASE q = k
          <6>. /\ q \in ack'[p]
            ...
            BY <3>1, <3>3, <4>1, <5>1, AtMostOneHead, CVC4 DEF BasicInv, NetworkInv
          <6>. QED  OBVIOUS
        <5>2. CASE q # k
          BY <3>1, <3>3, <4>1, <5>2
        <5>. QED  BY <5>1, <5>2
