...
(***************************************************************************)
EXTENDS Integers, TLAPS, FiniteSetTheorems, FunctionTheorems, Utils
...
(* There is no overlap on the stack ever: *)
...
---------------------------------------------------
\* Some lemmas about finite sets, cardinality, surjections, bijections
LEMMA FiniteSelfSurjectionIsBijection ==
  ASSUME NEW S, NEW f,
         IsFiniteSet (S),
         f \in Surjection (S, S)
  PROVE f \in Bijection (S, S)
  BY FS_SurjSameCardinalityImpliesInj DEF Bijection

LEMMA FS_BijSameCardinality ==
  ASSUME NEW S, NEW T,
         IsFiniteSet (S), IsFiniteSet(T),
         ExistsBijection(S,T)
  PROVE Cardinality(S) = Cardinality(T)
  <1>1 PICK f : f \in Bijection(1..Cardinality(S), S)
    BY FS_CardinalityType DEF ExistsBijection
  <1>2 PICK g : g \in Bijection (S, T)
    BY DEF ExistsBijection
  <1> DEFINE h == [i \in 1..Cardinality(S) |-> g[f[i]]]
  <1>3 h \in Bijection(1..Cardinality(S), T)
    BY <1>1, <1>2, Fun_BijTransitive
  <1> QED BY <1>3, FS_Bijection DEF ExistsBijection

LEMMA CardinalitySumDisjoint ==
  ASSUME NEW S, NEW T,
         IsFiniteSet(S), IsFiniteSet(T),
         S \cap T = {}
  PROVE Cardinality (S \cup T) = Cardinality (S) + Cardinality (T)
  <1>1 Cardinality (S \cap T) = 0 BY FS_EmptySet
  <1>2 Cardinality (S \cup T) = Cardinality (S) + Cardinality (T) - 0
    BY FS_Union, <1>1
  <1> QED BY <1>2, FS_CardinalityType
...
LEMMA PartitionsSetStable == \A B, C \in [1..N -> Int] : \A b, p, t \in 1..N :
  ...
  <1> SUFFICES ASSUME NEW x
               PROVE x \in {C[i] : i \in b..t} <=> x \in {B[i] : i \in b..t}
    OBVIOUS
  <1> DEFINE Cx == {i \in 1..N : C[i] = x}
             Bx == {i \in 1..N : B[i] = x}
  <1> Bx \subseteq 1..N /\ Cx \subseteq 1..N OBVIOUS
  <1> IsFiniteSet (Bx) /\ IsFiniteSet (Cx)
    BY FS_Interval, IsFiniteSet (1..N),  FS_Subset
  <1> /\ IsFiniteSet (Bx \ b..t) /\ IsFiniteSet (Bx \cap b..t)
      /\ IsFiniteSet (Cx \ b..t) /\ IsFiniteSet (Cx \cap b..t)
    BY FS_Difference, FS_Intersection
  <1>1 Cardinality (Cx) = Cardinality (Bx)
    <2>1 \E f : f \in Bijection(Cx, Bx)
      <3>1 PICK f \in Surjection(1..N, 1..N) : C = [i \in 1..N |-> B[f[i]]]
        BY DEF Partitions, Surjection, PermsOf
      <3>2 f \in Bijection(1..N,1..N)
        BY FiniteSelfSurjectionIsBijection, FS_Interval
      <3>3 Range (Restrict (f, Cx)) = Bx
        <4>1 Cx = {i \in 1..N : B[f[i]] = x} BY <3>1
        <4>2 \A i \in 1..N : f[i] \in 1..N
          BY <3>2, Fun_BijectionProperties, f \in [1..N -> 1..N]
        <4>3 \A i \in 1..N : i \in Cx <=> f[i] \in Bx
          BY <4>1, <4>2
        <4> HIDE DEF Bx, Cx
        <4>4 Cx = {i \in 1..N : f[i] \in Bx} BY <4>3
        <4>5 Range(Restrict(f, Cx)) = {f[i] : i \in Cx}
          BY DEF Range, Restrict
        <4>6 Range(Restrict(f, Cx)) \subseteq Bx
          BY <4>4, <4>5
        <4>7 ASSUME NEW k \in Bx
             PROVE k \in Range(Restrict(f, Cx))
          <5>1 SUFFICES \E i \in Cx : f[i] = k BY <4>5
          <5>2 B[k] = x BY <4>7 DEF Bx
          <5>3 PICK i \in 1..N : f[i] = k BY DEF Surjection
          <5>4 C[i] = x BY <3>1, <5>3, <5>2
          <5>5 i \in Cx BY <5>4 DEF Cx
          <5> QED BY <5>3, <5>5
        <4> QED BY <4>6, <4>7
      <3>4 Restrict(f, Cx) \in Bijection(Cx, Bx)
        BY <3>2, <3>3, Fun_BijRestrict
      <3> QED BY <3>4
    <2> QED BY FS_Bijection, <2>1 DEF ExistsBijection
  <1>2 Cardinality (Cx \ b..t) = Cardinality (Bx \ b..t)
    <2>1 SUFFICES Cx \ b..t = Bx \ b..t OBVIOUS
    <2>2 \A i \in Int : i \in 1..N \ b..t <=> i \in 1..(b-1) \cup (t+1)..N
      OBVIOUS
    <2>3 \A i \in 1..(b-1) \cup (t+1)..N : C[i] = B[i] BY DEF Partitions
    <2>4 \A i \in 1..N \ b..t: C[i] = B[i] BY <2>2, <2>3
    <2> QED BY <2>4
  <1>3 Cardinality (Cx \cap b..t) = Cardinality (Bx \cap b..t)
    <2>1 Cardinality (Cx) = Cardinality (Cx \ b..t) + Cardinality (Cx \cap b..t)
      BY CardinalitySumDisjoint
    <2>2 Cardinality (Bx) = Cardinality (Bx \ b..t) + Cardinality (Bx \cap b..t)
      BY CardinalitySumDisjoint
    <2> HIDE DEF Cx, Bx
    <2> QED BY <2>1, <2>2, <1>1, <1>2, FS_CardinalityType
  <1>4 x \in {C[i] : i \in b..t} <=> Cardinality (Cx \cap b..t) # 0
    <2>1 x \in {C[i] : i \in b..t} <=> \E i \in b..t : C[i] = x
      OBVIOUS
    <2>2 Cx \cap b..t = {i \in 1..N \cap b..t : C[i] = x} OBVIOUS
    <2>3 1..N \cap b..t = b..t OBVIOUS
    <2>4 Cx \cap b..t = {i \in b..t : C[i] = x} BY <2>2, <2>3
    <2> QED BY <2>1, <2>4, FS_EmptySet
  <1>5 x \in {B[i] : i \in b..t} <=> Cardinality (Bx \cap b..t) # 0
    <2>1 x \in {B[i] : i \in b..t} <=> \E i \in b..t : B[i] = x
      OBVIOUS
    <2>2 Bx \cap b..t = {i \in 1..N \cap b..t : B[i] = x} OBVIOUS
    <2>3 1..N \cap b..t = b..t OBVIOUS
    <2>4 Bx \cap b..t = {i \in b..t : B[i] = x} BY <2>2, <2>3
    <2> QED BY <2>1, <2>4, FS_EmptySet
