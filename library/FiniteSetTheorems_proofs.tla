...
BY FS_NatSurjection, Fun_NatSurjEquivNatBij, Zenon
  ...
  <1>1. IsCS(SZ)
    ...
    <2>1. \A i \in Nat : Prop(i)
      ...
      <3>2. ASSUME NEW i \in Nat, Prop(i)  PROVE Prop(i+1)
        ...
        (* Inductive step.                                                 *)
        (*******************************************************************)
        <4>. SUFFICES ASSUME NEW T \in SUBSET S, ExistsBijection(1 .. i+1,T)
                      PROVE  SZ[T] = fn(SZ,T)
          OBVIOUS
        <4>1. ~ExistsBijection(1..0,T)  BY Fun_NatBijSame, Z3
        <4>2. T # {}  BY <4>1, Fun_NatBijEmpty
        <4>3. Size(T) = i+1  BY Fun_NatBijSame
        <4>4. PICK t \in T : t = CHOOSE x : x \in T  BY <4>2
        <4>. DEFINE U == T \ {t}
        <4>5. ExistsBijection(1..i,U)  BY Fun_NatBijSubElem, Z3
        <4>6. SZ[U] = fn(SZ,U)  BY <4>5, <3>2
    ...
    <2> SUFFICES ASSUME NEW T \in SUBSET S  PROVE SZ[T] = fn(SZ,T)  BY Zenon DEF SZ
    ...
    <2>1. \A i \in Nat : Prop(i)
      ...
      <3>2. ASSUME NEW i \in Nat, Prop(i)  PROVE Prop(i+1)
        ...
        <4>. SUFFICES ASSUME NEW T \in SUBSET S, ExistsBijection(1 .. i+1,T)
             PROVE CS1[T] = CS2[T]
              OBVIOUS
        <4>5. ~ExistsBijection(1..0,T)  BY Fun_NatBijSame, Z3
...
<1>2. PICK n \in Nat : ExistsBijection(1..n, T) /\ n <= Cardinality(S)
  BY <1>1, Fun_NatBijSubset, Zenon
<1>3. ASSUME Cardinality(S) = Cardinality(T), S # T
      PROVE  FALSE
  <2>1. PICK x \in S \ T : TRUE  BY <1>3, Zenon
  <2>2. /\ IsFiniteSet(S \ {x})
  ...
  <2>4. PICK m \in Nat : ExistsBijection(1..m, T) /\ m <= Cardinality(S)-1
    BY <2>2, <2>3, FS_CardinalityType, Fun_NatBijSubset, Zenon
...
(* `.  .'                                                                  *)
(*                                                                         *)
(* Symmetric theorem for injections and the pigeonhole principle.          *)
(*                                                                         *)
(* `.  .'                                                                  *)
(***************************************************************************)
THEOREM FS_Injection ==
  ASSUME NEW S, NEW T, NEW f \in Injection(S,T), IsFiniteSet(T)
  PROVE  /\ IsFiniteSet(S)
         /\ Cardinality(S) <= Cardinality(T)
...
<1>2. CASE S # {}
  ...
  <2>3. ASSUME Cardinality(S) = Cardinality(T)
        PROVE  f \in Surjection(S,T)
    <3>. SUFFICES ASSUME NEW t \in T, \A s \in S : f[s] # t
                  PROVE  FALSE
      BY DEF Surjection, Injection
    <3>1. ExistsBijection(T,S)
      BY <2>1, <2>2, <2>3 DEF ExistsBijection, Bijection
    <3>2. f \in Bijection(S, Range(f))
      BY Fun_InjMeansBijImage
    <3>3. ExistsBijection(T, Range(f))
      BY <3>1, <3>2, Fun_ExistsBijTransitive DEF ExistsBijection
    <3>4. Cardinality(T) = Cardinality(Range(f))
      BY <3>3, FS_Bijection
    <3>5. Range(f) \subseteq T
      BY DEF Injection, Range
    <3>6. Range(f) = T
      BY <3>4, <3>5, FS_Subset, Zenon
    <3>. QED  BY <3>6 DEF Range, Injection
  <2>4. ASSUME f \in Surjection(S,T)
        PROVE  Cardinality(S) = Cardinality(T)
    BY <2>2, <2>4, FS_Surjection, FS_CardinalityType
  <2>. QED  BY <2>2, <2>3, <2>4
<1>. QED  BY <1>1, <1>2

THEOREM FS_PigeonHole ==
  ASSUME NEW S, IsFiniteSet(S),
...
THEOREM FS_Induction ==
  ASSUME NEW S, IsFiniteSet(S),
         ...
         PROVE  P(T \cup {x})
  PROVE  P(S)
<1>.  DEFINE Q(n) == \A T \in SUBSET S : Cardinality(T) = n => P(T)
<1>1. SUFFICES \A n \in Nat : Q(n)  BY FS_CardinalityType
<1>2. Q(0)  BY FS_EmptySet, FS_Subset
<1>3. ASSUME NEW n \in Nat, Q(n),
             NEW T \in SUBSET S, Cardinality(T) = n+1
      PROVE  P(T)
  <2>1. PICK x \in T : TRUE  BY <1>3, FS_EmptySet, FS_Subset
  <2>2. /\ T \ {x} \in SUBSET S
        /\ IsFiniteSet(T \ {x})
        /\ Cardinality(T \ {x}) = n
    BY <1>3, FS_RemoveElement, FS_Subset
  <2>3. P(T \ {x})  BY <2>2, Q(n)
  <2>. QED  BY <2>2, <2>3, Zenon
...
<1>. QED  BY <1>2, <1>3, IsWellFoundedOnSubrelation, Zenon
...
<1>2. ASSUME NEW A \in SUBSET T, IsFiniteSet(A), P(A), NEW x \in T \ A
      PROVE  P(A \cup {x})
  ...
  <2>. /\ IsFiniteSet(S \cup A)
    ...
    BY P(A), FS_Subset, FS_CardinalityType, Zenon
...
<1>1. P({})  BY FS_EmptySet, Zenon
<1>2. ASSUME NEW U \in SUBSET S, P(U), NEW x \in S \ U
      PROVE  P(U \cup {x})
  BY <1>2, FS_Union, Isa
<1>. HIDE DEF P
...
<1>2. ASSUME NEW A \in SUBSET T, IsFiniteSet(A), P(A), NEW x \in T \ A
  ...
  <2>. /\ Cardinality(A) \in Nat
       /\ Cardinality(S) \in Nat
    BY <1>2, FS_CardinalityType, FS_Subset
...
<1>2. ASSUME NEW A \in SUBSET S, IsFiniteSet(A), P(A), NEW x \in S \ A
  ...
  <2>4. ExistsBijection(SUBSET A, Ax)
    ...
    <3>1. f \in Injection(SUBSET A, Ax)
      <4>1. f \in [SUBSET A -> Ax]
        BY Zenon
      <4>2. ASSUME NEW B \in SUBSET A, NEW C \in SUBSET A, f[B] = f[C]
          PROVE  B = C
        BY <4>2, <1>2, Zenon
      <4>. QED  BY <4>1, <4>2, Fun_IsInj, Zenon
    <3>2. f \in Surjection(SUBSET A, Ax)  BY Zenon DEF Surjection
    <3>3. f \in Bijection(SUBSET A, Ax)
      BY <3>1, <3>2 DEF Bijection
    <3>. QED  BY <3>3, Zenon DEF ExistsBijection
  <2>5. /\ IsFiniteSet(Ax)
