...
<1>2. \A n \in Nat : <<f[n+1], f[n]>> \in RR \cap (S \X S)
  BY <1>1
<1>. QED  BY <1>2 DEF IsWellFoundedOn
...
<1>1. SUFFICES ASSUME NEW g, NEW h,
...
<1>2. ASSUME NEW x \in S,
  ...
  <2>1. Def(g,x) = Def(h,x)
    BY <1>2 DEF WFDefOn
  <2>2. QED
    BY <1>1, <2>1 DEF WFInductiveDefines
<1>3. QED
  BY <1>2, WFInduction, Isa
...
<1>2. U \subseteq TransitiveClosureOn(R,S)
  <2> SUFFICES ASSUME NEW x \in S, NEW y \in S, <<x,y>> \in U
    ...
    BY IsaM("blast")
  <2> QED
    BY TransitiveClosureThm, Zenon DEF IsTransitivelyClosedOn
...
<1>3. ASSUME NEW t \in T,
  ...
  <2>2. CASE t[1] = x1 /\ << t[2], x2 >> \in R2
    BY <1>2, <2>2
  <2>3. QED
    BY <2>1, <2>2, <1>3 DEF LexPairOrdering
<1> QED
  BY <1>3
...
<1>2. ASSUME NEW m \in Nat, P(m)
  ...
  <2>2. IsWellFoundedOn(PreImage(g, [1..m+1 -> S], LexPairOrdering(LPO(m), R, [1..m -> S], S)),
                         [1..m+1 -> S])
    <3>1. \A ss \in [1..m+1 -> S] : g(ss) \in [1..m -> S] \X S
      OBVIOUS
    <3> HIDE DEF g
    <3>2. QED
      BY <2>1, <3>1, PreImageWellFounded, Isa
  <2>3. LPO(m+1) = PreImage(g, [1..m+1 -> S], LexPairOrdering(LPO(m), R, [1..m -> S], S))
    <3>0. 1 .. m+1 = 1..m \union {m+1}
      OBVIOUS
      ...
      <4>2. << g(x), g(y) >> \in LexPairOrdering(LPO(m), R, [1..m -> S], S)
        <5>1. CASE j \in 1..m
          <6>1. << g(x)[1], g(y)[1] >> \in LPO(m)
            BY <3>0, <4>1, <5>1, Zenon DEF LexProductOrdering
          <6>2. QED
    ...
    <3>2. PreImage(g, [1..m+1 -> S], LexPairOrdering(LPO(m), R, [1..m -> S], S)) \subseteq LPO(m+1)
      ...
      <4>1. CASE << g(x)[1], g(y)[1] >> \in LPO(m)
        ...
        <5>1. PICK j \in 1..m : /\ \A i \in 1..j-1 : g(x)[1][i] = g(y)[1][i]
          ...
          BY <4>1, Zenon DEF LexProductOrdering
        <5>3. /\ g(x)[1][j] = x[j]
          ...
          BY DEF g
        <5> QED
          BY <5>1, <5>3, <3>0, Zenon DEF LexProductOrdering
