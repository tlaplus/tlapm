...
let rec lsp_stream_handler input_chan output_chan state =
  ...
  | Ok (Some packet) ->
      let state' =
        Tlapm_lsp_packets.handle_jsonrpc_packet packet writer state
      in
      lsp_stream_handler input_chan output_chan state'
...
let flow_handler trace input_flow output_flow =
  ...
  let write_fun buf_w =
    ...
    lsp_stream_handler input_chan output_chan Tlapm_lsp_state.empty
  in
